...
fn parse_vml_path_commands(s: Option<String>) -> Vec<VmlPathCommand> {
    ...
    while !rest.is_empty() {
        ...
        if cmd_len > 0 {
            ...
            let end = rest
                .find(|c: char| c == ',' || c.is_ascii_whitespace() || c.is_ascii_alphabetic())
                .unwrap_or(rest.len());
            if end > 0 {
    ...
    while i < tokens.len() {
        ...
        match tok {
            "m" => {
                if let Some((x, y)) = take_2_i64(&tokens, &mut i) {
            ...
            "c" => {
                if let Some((x1, y1, x2, y2, x, y)) = take_6_i64(&tokens, &mut i) {
            ...
            "r" => {
                if let Some((dx, dy)) = take_2_i64(&tokens, &mut i) {
                    cmds.push(VmlPathCommand::RLineTo { dx, dy });
                }
            }
            "v" => {
                if let Some((dx1, dy1, dx2, dy2, dx, dy)) = take_6_i64(&tokens, &mut i) {
            ...
            "wa" | "wr" | "at" | "ar" => {
                ...
                let args = (|| {
                    Some(VmlPathCommand::Arc {
                        kind,
                        bounding_x1: take_i64(&tokens, &mut i)?,
                        bounding_y1: take_i64(&tokens, &mut i)?,
                        bounding_x2: take_i64(&tokens, &mut i)?,
                        bounding_y2: take_i64(&tokens, &mut i)?,
                        start_x: take_i64(&tokens, &mut i)?,
                        start_y: take_i64(&tokens, &mut i)?,
                        end_x: take_i64(&tokens, &mut i)?,
                        end_y: take_i64(&tokens, &mut i)?,
                    })
                })();
...
fn take_i64(tokens: &[&str], i: &mut usize) -> Option<i64> {
    if *i < tokens.len() {
        let v = tokens[*i].parse::<i64>().ok()?;
        *i += 1;
        Some(v)
    } else {
        None
...
fn take_2_i64(tokens: &[&str], i: &mut usize) -> Option<(i64, i64)> {
    let a = take_i64(tokens, i)?;
    let b = take_i64(tokens, i)?;
    Some((a, b))
}

fn take_6_i64(tokens: &[&str], i: &mut usize) -> Option<(i64, i64, i64, i64, i64, i64)> {
    let a = take_i64(tokens, i)?;
    let b = take_i64(tokens, i)?;
    let c = take_i64(tokens, i)?;
    let d = take_i64(tokens, i)?;
    let e = take_i64(tokens, i)?;
    let f = take_i64(tokens, i)?;
    Some((a, b, c, d, e, f))
}
