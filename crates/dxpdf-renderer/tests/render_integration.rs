//! Integration tests — parse real DOCX files with dxpdf-docx, render with dxpdf-renderer.

use std::path::Path;

const TEST_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../dxpdf-docx/test-files");
...
fn parse_docx(filename: &str) -> dxpdf_docx_model::model::Document {
    let path = Path::new(TEST_DIR).join(filename);
    let bytes = std::fs::read(&path).unwrap_or_else(|e| {
        panic!("Failed to read {}: {e}", path.display());
    });
    dxpdf_docx::parse(&bytes).unwrap_or_else(|e| {
...
fn all_files_resolve_without_error() {
    for filename in test_docx_files() {
        ...
        let resolved = dxpdf_renderer::resolve::resolve(&doc);
        assert!(
...
fn all_files_layout_without_error() {
    for filename in test_docx_files() {
        let doc = parse_docx(filename);
        let (_, pages) = dxpdf_renderer::resolve_and_layout(&doc);
        assert!(
            !pages.is_empty(),
...
fn all_files_render_to_pdf() {
    ...
    for filename in test_docx_files() {
        let doc = parse_docx(filename);
        let pdf_bytes = dxpdf_renderer::render_with_font_mgr(&doc, &font_mgr)
            ...
            .map(|p| {
                p.commands
                    ...
                    .filter(|c| {
                        matches!(
                            c,
                            dxpdf_renderer::layout::draw_command::DrawCommand::Text { .. }
                        )
                    })
                    .count()
            })
