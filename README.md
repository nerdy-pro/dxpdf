...
| `w:bidi` section layout (§17.6.6) | ✅ a right-to-left section's **leading** margin is its right, so §17.4.28 `w:jc`'s `start`/`end` and §17.4.50 `tblInd` resolve against it — `w:jc="left"` right-aligns a table, since Transitional `left` *is* Strict `start`. Distinct from §17.4.1 `w:bidiVisual`, which reverses cells inside one table. The section's RTL *paragraph* default and multi-column order are not wired |
