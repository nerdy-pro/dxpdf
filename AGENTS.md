...
| `page-borders.docx`, `doc-grid.docx` | Issue #151 — §17.6.10 page borders and the §17.6.5 applied line grid. The borders file runs the same four deliberately distinct edges (each its own colour, width and space) through both offset modes, one section `offsetFrom="page"`, one `"text"`, so each edge's band is pinned by colour and the pages differ only in reference frame. The grid file carries identical content twice: section 1 under `type="lines" linePitch="360"`, section 2 under the type-less `linePitch="360"` every Word document carries — the ungridded control the assertions measure against, so no font metric is pinned. Built by `scripts/make_issue151_fixtures.py`; exercised by `tests/page_borders.rs` and `tests/doc_grid.rs` |
