...
**No doc yet** — start from the module docs at these entry points: character spacing and distributed alignment (`src/render/spacing.rs` — §17.3.2.35 and §17.3.1.13 share one unit: the UAX #29 grapheme cluster on the cmap path, the shaped cluster on a shaped run; the module doc owns why and routes the three shaped askers), color-emoji pipeline (`src/render/emoji/mod.rs`), parse/serde schemas (`src/docx/parse/`, the `XxxXml` → domain seam), text shaping & fragments (`src/render/layout/fragment/`), per-glyph font fallback (`src/render/layout/fragment/fallback.rs` — why the fallback is carried as a name and not a resolved face, and why the early-out is load-bearing), paint & PDF emission (`src/render/painter.rs`), EMF images (`src/render/emf.rs`), numbering & list labels (`src/docx/parse/numbering.rs`, `src/render/layout/build/list_label.rs`), VML fallback (`src/docx/parse/vml/`).
...
| `devanagari.docx` | Issue #153 — complex-script shaping with the shaped cluster as the spacing unit: plain Devanagari (prebase matra, virama conjunct, reph) naming **no font anywhere** so per-glyph fallback must run before shaping; the same text under §17.3.2.35 `w:spacing`; a `jc=distribute` Devanagari line; a Latin `w:spacing` control that must stay off the shaping path; and Arabic under `distribute` — the regression pin for distribution stretching a shaped run's decorations while the painter ignored its glyphs. Built by `scripts/make_devanagari_fixture.py`; exercised by `tests/devanagari_shaping.rs`, with the matra-reorder itself pinned against a live face in `render::shape`'s unit tests |
