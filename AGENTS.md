...
| `text-effects.docx` | §17.3.2.13/.18/.23/.31 (issue #148) — one paragraph per legacy run effect plus a control: shadow on black and on red text (the copy's colour is keyed on luminance), outline, the permitted shadow+outline pair, emboss and imprint. Built by `scripts/make_issue148_fixture.py`; asserted copy-by-copy (count, offset direction, colours, draw order, stroke flag) by `tests/text_effects.rs` |
