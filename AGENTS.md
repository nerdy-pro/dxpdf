...
| `counting-*.docx` (8 files: zh-hans, zh-hant, ja, ko, vi, hi, th, en-dollar) | §17.18.59 counting systems (issue #152) — per language, every `w:numFmt` under test as two lists crossing the tens boundary (items 1–3 and 10–12) with a bare `%1` level text. Built by `scripts/make_issue152_fixtures.py`; asserted label-for-label by `tests/counting_systems.rs` |
