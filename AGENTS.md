...
| `bidi-tabs.docx` | §17.3.1.37 under `w:bidi` (issue #156) — four RTL/LTR paragraph pairs on one page: Strict-spelled `end`/`start` stops whose zones land metric-free at `540−pos` / `72+pos`, numbered items whose labels must sit against their leading margin (RTL start=7, LTR start=3 — distinct label strings), a mirrored `bar` rule, and the default grid walked from the right. Built by `scripts/make_issue156_fixture.py`; asserted position-by-position by `tests/bidi_tabs.rs` |
