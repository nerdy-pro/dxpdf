...
| `bidi-visual.docx` | §17.4.1 `w:bidiVisual` (issue #157) — three tables over one lopsided 72/144/216 pt grid: a mirrored row, a mirrored table with `gridSpan`, `vMerge` and borders, and an unflagged control. The lopsided grid is what separates a genuine column mirror from a mere reversal of the text. Built by `scripts/make_issue157_fixture.py`; asserted position-by-position by `tests/bidi_visual.rs` |
