...
| `wmf-image.docx`, `svg-image.docx` | Issue #150 — the WMF one wraps a hand-built placeable WMF (`META_DIBSTRETCHBLT` around a 2×2 24-bpp DIB with four distinct quadrant colours); the SVG one pairs Word's shape (blue PNG fallback in the main blip, red SVG in the `svgBlip` extension — the drawn colour says which part was picked) with a no-fallback blip pointing straight at a green SVG. Built by `scripts/make_issue150_fixtures.py`; exercised by `tests/wmf_svg_images.rs`, with decode-level pixel assertions in `render::dib`/`render::wmf`/`render::svg` unit tests |
