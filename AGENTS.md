...
| `shading-patterns.docx` | §17.18.78 `ST_Shd` (issue #149) — one borderless table, one `w:shd` value per cell with colours unique across the fixture: the tint/`solid`/`clear`/`nil` flat family (including the eighth-percentage `pct12` and the all-`auto` `pct50`), and one of each geometric stripe/cross family plus a `thin` variant. `solid`'s fill and `nil`'s colour are the two colours no cell may produce. Built by `scripts/make_issue149_fixture.py`; asserted colour-by-colour by `tests/shading_patterns.rs` |
