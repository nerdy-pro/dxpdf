...
| `custgeom-arc.docx` | §20.1.9.3 `a:arcTo` inside `<a:custGeom>` — a quarter pie, an ellipse chord with wR≠hR at non-axis angles (the ray→parametric angle skew), a single 360° swing (the full disc arc APIs erase when unsplit), and a crescent chaining two arcs through the shared pen position. Built by `scripts/make_custgeom_arc_fixture.py`; parse-asserted in `tests/parse_test_files.rs`, painter conversion pinned by the §20.1.9.3 unit tests in `src/render/painter.rs`. LibreOffice render is the reference |
...
| `smartart.docx`, `charts.docx` | Issue #155 — SmartArt via the pre-laid-out `dsp:` drawing part (three `roundRect` nodes + a `rightArrow`, absolute-EMU canvas, `dsp:txXfrm` label rects, wired through the document's rels exactly as Word wires it — including the MS-ODRAWXML quirk that the data part's `dataModelExt/@relId` resolves against the *host* rels), and three chart parts (clustered columns with title/legend/gridlines, a varyColors pie, a line with a blank-cell gap and markers). Both carry the stock Office theme so accent RGBs are deterministic literals in the tests. Built by `scripts/make_issue155_fixtures.py`; exercised by `tests/smartart_charts.rs` |
