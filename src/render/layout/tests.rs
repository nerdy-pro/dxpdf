...
fn layout_empty_document() {
    ...
    let pages = layout(&doc, &LayoutConfig::default(), &test_font_mgr());
...
fn layout_single_paragraph() {
    ...
    let pages = layout(&doc, &config, &test_font_mgr());
    assert_eq!(pages.len(), 1);
    assert!(!pages[0].commands.is_empty());
    assert!(pages[0]
        .commands
...
fn right_alignment() {
    ...
    assert!(
        text_x > config.margin_left + config.content_width() / 2.0,
...
fn empty_paragraph_still_has_height() {
    ...
    assert!(
        after_y > config.margin_top + 10.0,
...
fn float_adjustment_shifts_text() {
    ...
    assert!(
        text.0 > config.margin_left + 100.0,
        "Text x={} should be right of float (margin+100={})",
        text.0,
        config.margin_left + 100.0
    );
}
...
fn after_table_spacing_uses_doc_default() {
    ...
    let pages_with = layout(&doc, &LayoutConfig::default(), &test_font_mgr());
    ...
    let pages_without = layout(&doc_no_sp, &LayoutConfig::default(), &test_font_mgr());
