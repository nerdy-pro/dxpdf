                ...
                Fragment::MathFraction {
                    ...
                    for (row, row_baseline) in [(num, num_baseline), (den, den_baseline)] {
                        commands.push(DrawCommand::Text {
                            ...
                            shaped: None,
                            // §17.3.2.23: an equation's rows are not a run, so
                            // no legacy outline effect applies to them.
                            outline: false,
                        });
                    }
                    commands.push(DrawCommand::Line {
...
mod tests {
    use super::{
        compute_line_placements, distribution_unit_count, emit_line_commands, find_next_tab_stop,
        resolve_ptab, resolve_zone_anchor, DrawCommand, Fragment, LineLayoutParams, LinePen,
        PTabGeometry, PTabPlacement, ParagraphStyle, ZoneAnchor,
    };
    use crate::model;
    use crate::render::dimension::Pt;
