//! Legacy event-driven body parser, kept alive for three mid-stream callers
//! that read `<w:p>` / `<w:tbl>` / block content from inside a larger event
//! stream: `drawing/shape.rs` text boxes, and `vml/mod.rs` text boxes. The
//! public byte-level `parse_body` / `parse_blocks` API lives in `body.rs`
//! (serde-based).
//!
//! This file will be retired when Phase 5 (DrawingML) and Phase 6 (VML)
//! migrate to serde.
//!
...
/// (Legacy, unused) Parse a body-level XML part. Byte-level callers should
/// use `body::parse_blocks` instead.
#[allow(dead_code)]
...
/// (Legacy, unused) Byte-level callers should use `body::parse_body` instead.
#[allow(dead_code)]
